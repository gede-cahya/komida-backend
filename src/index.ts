...
import { manga as mangaTable, siteVisits, mangaViews, decorations as decorationsTable, userDecorations as userDecorationsTable, badges as badgesTable, userBadges as userBadgesTable, transactions, userCredits } from './db/schema'
...
app.post('/api/auth/register', zValidator('json', registerSchema), async (c) => {
    try {
        ...
        // Give new users 500 free credits
...
// Auth Middleware for Shop routes (only /purchase requires auth)
app.use('/api/shop/purchase', async (c, next) => {
    ...
    c.set('user' as any, { id: payload.id, username: payload.username, role: payload.role });
    await next();
});
    ...
    try {
        ...
        if (credits.length === 0) {
            ...
            const [newCredits] = await db.insert(userCredits)
                .values({ userId, balance: 0, baseChainBalance: '0' })
                .returning();
            return c.json({ credits: newCredits });
        }

        return c.json({ credits: credits[0] });
    } catch (e: any) {
        return c.json({ error: e.message }, 500);
    }
});

app.get('/api/user/inventory', async (c) => {
    try {
        const userId = c.get('userId' as any) as number;

        // Get user decorations
        const userDecos = await db.select({
            item_id: decorationsTable.id,
            item_type: sql<string>`'decoration'`,
        ...
        const userBadges = await db.select({
            item_id: badgesTable.id,
            item_type: sql<string>`'badge'`,
            name: badgesTable.name,
            image_url: badgesTable.image_url,
        ...
        const inventory = [...userDecos, ...userBadges];
...
app.get('/api/user/transactions', async (c) => {
    try {
        const userId = c.get('userId' as any) as number;
